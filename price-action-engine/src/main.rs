...
struct Ema20CrossModelCn;

impl PriceActionModel for Ema20CrossModelCn {
    fn code(&self) -> &str { "pa_ema20_cross_cn_v1" }
    fn name(&self) -> &str { "pricedog_pa_ema20_cross_cn" }
    fn version(&self) -> &str { "v1" }
    fn min_klines(&self) -> usize { 22 }
...
/// A 股 EMA20 穿越检测：上穿看多提醒，下传看空提醒
fn detect_ema20_cross_cn(klines: &[Kline]) -> Vec<Signal> {
    if klines.len() < 22 {
        return Vec::new();
    }
    let curr = klines.last().unwrap();
    ...
    let (direction, alert_hint, expected_use, reason) = if crossed_up {
        ("long",
         "bullish_watch",
         "看多提醒：价格上穿EMA20，关注买入机会，下一根K线确认后可考虑建仓/加仓",
         format!("EMA20上穿: prev_c={:.4} prev_ema={:.4} curr_c={:.4} curr_ema={:.4}", prev.close, prev_ema20, curr.close, curr_ema20))
    } else if crossed_down {
        ("short",
         "bearish_watch",
         "看空提醒：价格下传EMA20，注意风险，考虑减仓/止盈，A股不可做空",
         format!("EMA20下传: prev_c={:.4} prev_ema={:.4} curr_c={:.4} curr_ema={:.4}", prev.close, prev_ema20, curr.close, curr_ema20))
    } else {
        return Vec::new();
    };
