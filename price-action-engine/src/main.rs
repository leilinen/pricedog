...
async fn main() -> Result<()> {
    ...
    let app = Router::new()
        ...
        .route("/api/v1/evaluate-rule", post(evaluate_rule))
...
// ---------------------------------------------------------------------------
// POST /api/v1/evaluate-rule — batch condition evaluation for price alerts
// ---------------------------------------------------------------------------

fn op_eval(left: Option<f64>, op: &str, right: &Value) -> bool {
    let Some(left) = left else { return false };
    let o = op.trim().to_ascii_lowercase();
    if o == "between" || o == "in" {
        let Some(arr) = right.as_array() else { return false };
        if arr.len() != 2 { return false; }
        let lo = arr[0].as_f64();
        let hi = arr[1].as_f64();
        match (lo, hi) {
            (Some(lo), Some(hi)) => lo <= left && left <= hi,
            _ => false,
        }
    } else {
        let Some(rv) = right.as_f64() else { return false };
        match o.as_str() {
            ">" => left > rv,
            ">=" => left >= rv,
            "<" => left < rv,
            "<=" => left <= rv,
            "=" | "==" => (left - rv).abs() < f64::EPSILON,
            "!=" | "<>" => (left - rv).abs() >= f64::EPSILON,
            _ => false,
        }
    }
...
async fn evaluate_rule(
    ...
    let default_interval = normalize_interval(&req.interval);

    // Collect which intervals need kline data
    let mut interval_set: HashMap<String, bool> = HashMap::new();
    for cond in &req.conditions {
        let ct = cond.cond_type.trim().to_ascii_lowercase();
        if ct == "volume_ratio" || ct == "ema20_position" || ct == "pattern" || ct == "ema20_cross" {
            let iv = cond.interval.as_deref().unwrap_or(&default_interval).to_string();
            interval_set.entry(iv).or_insert(false);
        }
    }

    // Fetch klines and compute indicators/signals per interval
    let mut kline_data: HashMap<String, Vec<Kline>> = HashMap::new();
    let mut indicator_data: HashMap<String, Indicators> = HashMap::new();
    let mut signal_data: HashMap<String, Vec<Signal>> = HashMap::new();
    let mut ema20_cross_data: HashMap<String, Vec<Signal>> = HashMap::new();

    for iv in interval_set.keys() {
        match fetch_klines(&state, &market, &symbol, iv, 120, false).await {
            Ok(klines) => {
                if klines.len() >= 25 {
                    let indicators = compute_indicators(&klines);

                    // Detect pattern signals (signal_bar model)
                    let needs_pattern = req.conditions.iter().any(|c| {
                        c.cond_type.trim().to_ascii_lowercase() == "pattern"
                            && c.interval.as_deref().unwrap_or(&default_interval) == *iv
                    });
                    let signals = if needs_pattern {
                        let model_code = if market == "CN" { "pa_signal_bar_cn_v1" } else { "pa_signal_bar_v1" };
                        resolve_model(model_code)
                            .map(|m| m.detect(&klines))
                            .unwrap_or_default()
                    } else {
                        Vec::new()
                    };

                    // Detect ema20_cross signals
                    let needs_ema20_cross = req.conditions.iter().any(|c| {
                        c.cond_type.trim().to_ascii_lowercase() == "ema20_cross"
                            && c.interval.as_deref().unwrap_or(&default_interval) == *iv
                    });
                    let cross_signals = if needs_ema20_cross {
                        let model_code = if market == "CN" { "pa_ema20_cross_cn_v1" } else { "pa_ema20_cross_v1" };
                        resolve_model(model_code)
                            .map(|m| m.detect(&klines))
                            .unwrap_or_default()
                    } else {
                    ...
                    kline_data.insert(iv.clone(), klines);
                    indicator_data.insert(iv.clone(), indicators);
                    signal_data.insert(iv.clone(), signals);
                    ema20_cross_data.insert(iv.clone(), cross_signals);
                }
            }
            Err(e) => {
                warn!("evaluate-rule: fetch_klines failed {}/{} {}: {}", market, symbol, iv, e);
            }
        }
    }
    ...
    for cond in &req.conditions {
        let ct = cond.cond_type.trim().to_ascii_lowercase();
        let op = cond.op.trim();
        let iv_key = cond.interval.as_deref().unwrap_or(&default_interval).to_string();

        let result = match ct.as_str() {
            "price" => {
                let left = req.quote.as_ref().and_then(|q| q.current_price);
                let matched = op_eval(left, op, &cond.value);
                ConditionResult {
                    cond_type: ct,
                    op: op.to_string(),
                    target: cond.value.clone(),
                    actual: left.map(Value::from),
                    matched,
                    signal: None,
                    error: None,
                }
            ...
            "change_pct" => {
                let left = req.quote.as_ref().and_then(|q| q.change_pct);
                let matched = op_eval(left, op, &cond.value);
                ConditionResult {
                    cond_type: ct,
                    op: op.to_string(),
            ...
            "turnover" => {
                let left = req.quote.as_ref().and_then(|q| q.turnover);
                let matched = op_eval(left, op, &cond.value);
                ConditionResult {
                    cond_type: ct,
                    op: op.to_string(),
            ...
            "volume" => {
                let left = req.quote.as_ref().and_then(|q| q.volume);
                let matched = op_eval(left, op, &cond.value);
                ConditionResult {
                    cond_type: ct,
                    op: op.to_string(),
            ...
            "volume_ratio" => {
                let left = indicator_data.get(&iv_key).and_then(|i| i.volume_ratio);
                let matched = op_eval(left, op, &cond.value);
                ConditionResult {
                    cond_type: ct,
                    op: op.to_string(),
            ...
            "ema20_position" => {
                let left = indicator_data.get(&iv_key).and_then(|i| i.ema20_position);
                let matched = op_eval(left, op, &cond.value);
                ConditionResult {
                    cond_type: ct,
                    op: op.to_string(),
            ...
            "pattern" => {
                let signals = signal_data.get(&iv_key).cloned().unwrap_or_default();
                let target = cond.value.as_str().unwrap_or("").trim().to_string();
                let matched_signal = signals.iter().find(|s| {
                    target.is_empty() || s.signal_type == target
                });
                ConditionResult {
                    cond_type: ct,
                    op: op.to_string(),
                    target: cond.value.clone(),
                    actual: matched_signal.as_ref().map(|s| Value::String(s.signal_type.clone())),
                    matched: matched_signal.is_some(),
                    signal: matched_signal.map(|s| serde_json::to_value(s).unwrap_or(Value::Null)),
                    error: None,
                }
            }
            "ema20_cross" => {
                let signals = ema20_cross_data.get(&iv_key).cloned().unwrap_or_default();
                let direction = cond.value.as_str().unwrap_or("any").trim().to_ascii_lowercase();
                let matched_signal = signals.iter().find(|s| {
                    let d = s.direction.trim().to_ascii_lowercase();
                    direction == "any"
                        || (direction == "up" && d == "long")
                        || (direction == "down" && d == "short")
                });
                ConditionResult {
                    cond_type: ct,
                    op: op.to_string(),
                    target: cond.value.clone(),
                    actual: matched_signal.as_ref().map(|s| Value::String(s.direction.clone())),
                    matched: matched_signal.is_some(),
                    signal: matched_signal.map(|s| serde_json::to_value(s).unwrap_or(Value::Null)),
                    error: None,
                }
            ...
            _ => ConditionResult {
                ...
                matched: false,
                signal: None,
                error: Some("unsupported_type".to_string()),
            },
        };
        results.push(result);
    }

    let matched = if req.group_op.trim().to_ascii_lowercase() == "or" {
